...
import type { Point, Annotation, ToolType } from '@/types'
...
export function useAnnotationDraw(
  ...
  const getPagePoint = useCallback(
    (e: React.MouseEvent): Point => {
      const pageEl = containerRef.current?.querySelector('.react-pdf__Page') as HTMLElement
        ?? containerRef.current?.querySelector('img') as HTMLElement
    ...
    (e: React.MouseEvent) => {
      ...
      if (activeTool === 'rectangle' || activeTool === 'ellipse') {
        ...
        setDrawState((s) => ({
          ...
          drawingAnnotation: {
            type: 'freehand',
            points: [...((s.drawingAnnotation as any)?.points ?? [start]), pt],
            color: activeColor,
            opacity: activeOpacity,
            strokeWidth: activeStrokeWidth,
          },
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (ann && start) {
      if ((activeTool === 'rectangle' || activeTool === 'ellipse') && (ann as any).rect) {
        const r = (ann as any).rect
        ...
        const a = ann as any
        const dx = (a.end?.x ?? 0) - (a.start?.x ?? 0)
        const dy = (a.end?.y ?? 0) - (a.start?.y ?? 0)
  ...
  return {
    drawingAnnotation: drawState.drawingAnnotation,
    isDragging: drawState.isDragging,
    handleMouseDown,
    handleMouseMove,
    handleMouseUp,
  }
