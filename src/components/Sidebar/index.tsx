...
import { Trash2, MessageSquare, Layers, List, Search, X, Tag } from 'lucide-react'
...
export function Sidebar() {
  ...
  const filtered = annotations.filter((a) => {
    ...
    return label.includes(s) || comment.includes(s)
  })
      ...
      <div className="flex items-center gap-0.5 p-2 border-b border-[var(--border)]">
        {TABS.map((tab) => (
          <button
            ...
            className={`
              flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium flex-1 justify-center
        ...
        {annotations.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-xs text-[var(--text-secondary)]">
              {search ? 'No matching annotations' : 'No annotations yet'}
            </p>
            {!search && (
      ...
      {pages.map((page) => (
        <button
          ...
          className={`
            ...
            ${page === currentPage
              ? 'bg-[var(--accent-dim)] text-[var(--accent)] border border-amber-500/20'
              : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-white/5'
            }
          `}
        >
          <span>Page {page}</span>
          <span className="badge bg-white/10 text-[var(--text-secondary)]">
            {byPage[page].length}
          </span>
        </button>
      ))}
  ...
  if (comments.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center py-12 text-center px-4">
        ...
        <p className="text-[11px] text-[var(--text-tertiary)] mt-1">
          Select an annotation and add a comment
        </p>
      </div>
    )
  }
    ...
    <div className="flex-1 overflow-y-auto p-2 space-y-2">
      {comments.map((ann) => (
        <div
          ...
          className={`
            p-2.5 rounded-lg cursor-pointer transition-all text-xs border
            ${selectedId === ann.id
              ? 'bg-[var(--accent-dim)] border-amber-500/20'
              : 'bg-white/3 border-[var(--border)] hover:border-[var(--border-strong)] hover:bg-white/5'
            }
          `}
        >
